...
void VideoCtl::ReadThread(VideoState *is)
{
    ...
    is->last_video_stream = is->video_stream = -1; //-1表示没有相应的流
    ...
    /*
        设置中断回调函数，如果出错或退出，就根据is->abort_request状态返回
        执行耗时操作时，会调用interrupt_callback.callback
    */
    ...
    for (;;) {
        ...
        /* if the queue are full, no need to read more */ //检查队列是否有足够的数据
        if (infinite_buffer<1 &&
                (is->audioq.size + is->videoq.size + is->subtitleq.size > MAX_QUEUE_SIZE  //总大小
        ...
        /* check if packet is in play range specified by user, then queue, otherwise discard *///播放范围内
...
VideoState* VideoCtl::stream_open(const char *filename)
{
    ...
    //构建读取线程
    is->read_tid = std::thread(&VideoCtl::ReadThread, this, is); //线程函数
...
void VideoCtl::video_display(VideoState *is)
{
    ...
    if (renderer)
    {
        ...
        if (g_show_rect_mutex.tryLock())
        {
            ...
            video_image_display(is);  //显示一帧的视频
