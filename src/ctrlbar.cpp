...
//    GlobalHelper::SetIcon(ui->speedBtn, 12, QChar(0xf013));
  ...
  //  m_SettingBtn_Rotate = new QPropertyAnimation(ui->SettingBtn, "rotation");
    m_SettingBtn_trans.rotate(180);
    return true;
...
bool CtrlBar::ConnectSignalSlots()
{
    ...
    connect(ui->PlaySlider, &CustomSlider::SigMouseMove,this,&CtrlBar::OnMouseOnPlaySlider);
    ...
    connect(ui->PlaySlider, &CustomSlider::SigMouseLeave,this,&CtrlBar::OnMouseLeave);
    return true;
}
...
void CtrlBar::OnMouseOnPlaySlider()
{
    int x=this->mapFromGlobal(QCursor().pos()).x();
    double percent = x / (double)ui->PlaySlider->width();
    int seconds= percent* m_nTotalPlaySeconds;
    qDebug()<<seconds;
    int thh, tmm, tss;
    thh = seconds / 3600;
    tmm = (seconds % 3600) / 60;
    tss = (seconds % 60);
    mdrumpTime->setText(tr("%1").arg(tss));//
    mdrumpTime->move(ui->PlaySlider->pos().x()+ui->PlaySlider->width()*percent,ui->PlaySlider->pos().y()-10);
    mdrumpTime->show();
   // qDebug()<<TotalTime;
  //  mdrumpTime->setTime(TotalTime);

}

void CtrlBar::OnMouseLeave()
{
    mdrumpTime->hide();
...
{
//    m_SettingBtn_Rotate->setDuration(4000);
//    m_SettingBtn_Rotate->setStartValue(0);
//    m_SettingBtn_Rotate->setEndValue(360);
//    m_SettingBtn_Rotate->start();
  //  ui->SettingBtn->setTransform()
    //emit SigShowSetting();
}
